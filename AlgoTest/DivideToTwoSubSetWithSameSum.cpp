...
//      优化的解法为动态规划，找出递推关系式。假设dp[i][j］为数组前i个元素能否得到和为j的子数组，dp[i][j]＝1为可得到；dp[i][j]为不可得到；对i来说，如果i是和为j的子数组中，则对i－1个元素，应该得到和为j－nums［i］的子数组；即d[i-1][j-nums[i]]; 如果i不在子数组中，则对i－1个元素，应该得到和为j的子数组；即d[i-1][j]
//      所以这个递推关系式为dp[i][j] = dp[i-1][j-nums[i]] || dp[i-1][j];
//
//      上面这个数组的大小为原数组size＊sum的数量；因为i只与i－1有关，所以可以用2个sum大小的数组，滚动存储得到结果。
//      然后，只要保证dp［j］中存储的是上一轮i－1迭代的结果，这样就只需要一个dp［j］数组，关系式为：dp[j] = dp[j] || dp[j-nums[i]]。注意：这里要计算dp［j］必须从大到小，这是因为j－nums［i］是一个比j小的值，而递推关系需要上一轮的dp值，所以必须从大到小，这样在计算大值时，可以用到上一轮的dp值。
//
