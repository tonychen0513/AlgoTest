...
//     右边点的最短路径D（lrmin），这时因为已经有D（rmin）和D（lmin），可知这个左右间最短路径不能大于dist＝Min（D（rmin），D（lmin）），以左边为例，即只需比较中间线＋dist以内的点，
//     有证据表明，这个区间最多只能包含6个点，因此左右间比较可以优化为O（n），最后整个区间的最短距离为D（rmin），D（lmin），D（lrmin）的最小值。
//
//
